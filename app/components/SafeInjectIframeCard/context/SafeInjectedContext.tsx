...
import { FallbackProvider, JsonRpcProvider, getAddress } from "ethers";
...
import { useEthersProvider } from "~/hooks/ethers-provider";
...
export const SafeInjectProvider: React.FunctionComponent<FCProps> = ({
  ...
  const { id: spaceChainId } = useChainConfigOfSpace();
  const ethersProvider = useEthersProvider();
  const provider =
    ((ethersProvider as FallbackProvider)?.providerConfigs?.[0]
      ?.provider as JsonRpcProvider) || (ethersProvider as JsonRpcProvider);
